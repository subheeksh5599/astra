...
def transmitter(env: dict, domain: int) -> str:
    override = env.get(f"TRANSMITTER_DOMAIN_{domain}")
    return override or TRANSMITTERS[deployment(env)][domain]


def messenger(env: dict, domain: int) -> str:
    override = env.get(f"MESSENGER_DOMAIN_{domain}")
    return override or MESSENGERS[deployment(env)][domain]
